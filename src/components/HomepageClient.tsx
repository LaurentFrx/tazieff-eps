...
export function HomepageClient({ exerciseCount, methodeCount, learnCount }: Props) {
  ...
  return (
    <section className="page">
      ...
      {/* Search bar (visual — tâche 0.4 pour la fonctionnalité) */}
      <Link
        href="/exercices"
        className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-[color:var(--muted)] backdrop-blur transition-colors hover:bg-white/10"
      >
        <IconSearch />
        <span>{t("pages.home.searchPlaceholder")}</span>
      </Link>
